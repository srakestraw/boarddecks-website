#!/bin/bash

# Script to set Netlify environment variables
# Make sure you're logged into Netlify CLI first: netlify login

echo "Setting Netlify environment variables..."

# Set each environment variable
netlify env:set GOOGLE_SERVICE_ACCOUNT_EMAIL "boarddecks@boarddecks-early-access.iam.gserviceaccount.com"

# For the private key, we need to handle it carefully
echo "Setting GOOGLE_PRIVATE_KEY..."
PRIVATE_KEY="-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"

netlify env:set GOOGLE_PRIVATE_KEY "$PRIVATE_KEY"

netlify env:set GOOGLE_SPREADSHEET_ID "1TrZEorP3JvvlXjY2wCmOh2T6RrBmh3apJtloOsy6mPk"

netlify env:set GOOGLE_SHEET_NAME "Early Access Signups"

echo "Environment variables set successfully!"
echo "You can verify by running: netlify env:list" 